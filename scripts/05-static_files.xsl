...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
            ...
            <revision>
                ...
                <text xml:space="preserve" bytes="3441">
                   <xsl:apply-templates select="./text"/>
                    <xsl:for-each select="./meta/categories/category"><xsl:if test=". != ''"> 
    ...
    <xsl:template match="a">[[<xsl:if test="./@linking = 'no'">:</xsl:if><xsl:value-of select="./@link"/> | <xsl:value-of select="."/> ]]</xsl:template>
    <xsl:template match="b">'''<xsl:apply-templates/>'''</xsl:template>
    <xsl:template match="i">''<xsl:apply-templates/>''</xsl:template>
    <xsl:template match="tab">; <xsl:apply-templates/></xsl:template>
</xsl:stylesheet>