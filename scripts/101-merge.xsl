...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="all" select="count(collection('../output/single?select=*.xml')//*:page)" />
    <xsl:template match="/">
        <mediawiki xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
            ...
            <xsl:for-each select="collection('../output/single?select=*.xml')//page" xpath-default-namespace="http://www.mediawiki.org/xml/export-0.10/">
                <xsl:copy>
